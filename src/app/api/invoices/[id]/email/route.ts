  ...
  try {
    ...
    const html = `
      <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#fff;">
        ...
        <h3 style="color:#374151">Bill To: ${invoice.vendor.company}</h3>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;">
          ...
          <tbody>${items.map(it => `<tr><td style="padding:8px;border-bottom:1px solid #f1f5f9">${it.name}</td><td style="padding:8px;text-align:right;border-bottom:1px solid #f1f5f9">${it.quantity}</td><td style="padding:8px;text-align:right;border-bottom:1px solid #f1f5f9">${fmt(it.totalPrice)}</td></tr>`).join('')}</tbody>
        </table>
